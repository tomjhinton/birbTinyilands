...
void main() {
 ...
 vec3 color = vec3(1.);
        ...
        color = mix(color, vec3(0., .8, 0.), step(cnoise(uv * 800. * uRandom), .2));
        ...
        vec3 beach = vec3(1., 1., uRandom);

        beach = mix(beach , vec3(.7,.7, uRandom), cnoise(uv * 1000. * uRandom));
     
      // color += color2.r * .2;


      color = mix( color, beach, step(length(uv-.5 + (uRandom * .2)), .4 + (uRandom * .3)));
